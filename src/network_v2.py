...
class Distributor(Node):
    def __init__(self):
        super().__init__()
        self.coverage = None
        self._demand = constant_fn(None)
        self._supply = constant_fn(None)
        self._cost = constant_fn(None)
        self.distance_fn = manhattan_dist
...
class Network:
    def __init__(self, nodes, edges = None):
        ...
        for edge in self.edges:
            self.adj[edge.node1.id][edge.node2.id] = edge.id
            self.adj[edge.node2.id][edge.node1.id] = edge.id
    ...
    def edges(self, edges):
        ...
        for edge in self.edges:
            ...
            if edge.node1.id in self.adj[edge.node2.id]:
                raise ValueError('edge nodes already connected')
            if edge.node2.id in self.adj[edge.node1.id]:
                raise ValueError('edge nodes already connected')
            self.adj[edge.node1.id][edge.node2.id] = edge.id
            self.adj[edge.node2.id][edge.node1.id] = edge.id
    ...
    def add_edge(self, node1,node2, dist_fn = manhattan_dist):
        if self.adj[node1.id][node2.id]:
            raise ValueError('edge already in network')
        if self.adj[node2.id][node1.id]:
            raise ValueError('edge already in network')
        edge = Edge.make(node1,node2,dist_fn)
        self.edges.append(edge)
        self._edge_translator[edge.id] = edge
        self.adj[node1.id][node2.id] = edge.id
        self.adj[node2.id][node1.id] = edge.id
    ...
    def remove_edge(self, edge):
        if edge in self.edges:
            ...
            del self._edge_translator[edge.id]
            del self.adj[edge.node1.id][edge.node2.id]
            del self.adj[edge.node2.id][edge.node1.id] 
        else:
